...
public class GraphDiagramEngineMetamodelFactory
{
	...
	public void setRAAPI(RAAPI _raapi, String prefix, boolean insertMetamodel) throws ElementReferenceException // set RAAPI to null to free references
	{
		...
		if (raapi != null) {
			...
			if (insertMetamodel) {
				if (!raapi.isDirectSubClass(OKCMD, COMMAND))
					if (!raapi.createGeneralization(OKCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes OkCmd and Command.");
					}
				if (!raapi.isDirectSubClass(POPUPCMD, COMMAND))
					if (!raapi.createGeneralization(POPUPCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes PopUpCmd and Command.");
					}
				if (!raapi.isDirectSubClass(ACTIVEDGRCMD, COMMAND))
					if (!raapi.createGeneralization(ACTIVEDGRCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ActiveDgrCmd and Command.");
					}
				if (!raapi.isDirectSubClass(ACTIVEDGRVIEWCMD, COMMAND))
					if (!raapi.createGeneralization(ACTIVEDGRVIEWCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ActiveDgrViewCmd and Command.");
					}
				if (!raapi.isDirectSubClass(PASTECMD, COMMAND))
					if (!raapi.createGeneralization(PASTECMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes PasteCmd and Command.");
					}
				if (!raapi.isDirectSubClass(UPDATEDGRCMD, COMMAND))
					if (!raapi.createGeneralization(UPDATEDGRCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes UpdateDgrCmd and Command.");
					}
				if (!raapi.isDirectSubClass(CLOSEDGRCMD, COMMAND))
					if (!raapi.createGeneralization(CLOSEDGRCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes CloseDgrCmd and Command.");
					}
				if (!raapi.isDirectSubClass(SAVEDGRCMD, COMMAND))
					if (!raapi.createGeneralization(SAVEDGRCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes SaveDgrCmd and Command.");
					}
				if (!raapi.isDirectSubClass(ACTIVEELEMENTCMD, COMMAND))
					if (!raapi.createGeneralization(ACTIVEELEMENTCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ActiveElementCmd and Command.");
					}
				if (!raapi.isDirectSubClass(AFTERCONFIGCMD, COMMAND))
					if (!raapi.createGeneralization(AFTERCONFIGCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes AfterConfigCmd and Command.");
					}
				if (!raapi.isDirectSubClass(SAVESTYLESCMD, COMMAND))
					if (!raapi.createGeneralization(SAVESTYLESCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes SaveStylesCmd and Command.");
					}
				if (!raapi.isDirectSubClass(STYLEDIALOGCMD, COMMAND))
					if (!raapi.createGeneralization(STYLEDIALOGCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes StyleDialogCmd and Command.");
					}
				if (!raapi.isDirectSubClass(DEFAULTSTYLECMD, COMMAND))
					if (!raapi.createGeneralization(DEFAULTSTYLECMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes DefaultStyleCmd and Command.");
					}
				if (!raapi.isDirectSubClass(REFRESHDGRCMD, COMMAND))
					if (!raapi.createGeneralization(REFRESHDGRCMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes RefreshDgrCmd and Command.");
					}
				if (!raapi.isDirectSubClass(UPDATESTYLECMD, COMMAND))
					if (!raapi.createGeneralization(UPDATESTYLECMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes UpdateStyleCmd and Command.");
					}
				if (!raapi.isDirectSubClass(REROUTECMD, COMMAND))
					if (!raapi.createGeneralization(REROUTECMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes RerouteCmd and Command.");
					}
				if (!raapi.isDirectSubClass(ALIGNSIZECMD, COMMAND))
					if (!raapi.createGeneralization(ALIGNSIZECMD, COMMAND)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes AlignSizeCmd and Command.");
					}
				if (!raapi.isDirectSubClass(POPUPELEMSELECTEVENT, EVENT))
					if (!raapi.createGeneralization(POPUPELEMSELECTEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes PopUpElemSelectEvent and Event.");
					}
				if (!raapi.isDirectSubClass(PASTEGRAPHCLIPBOARDEVENT, EVENT))
					if (!raapi.createGeneralization(PASTEGRAPHCLIPBOARDEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes PasteGraphClipboardEvent and Event.");
					}
				if (!raapi.isDirectSubClass(DELETECOLLECTIONEVENT, EVENT))
					if (!raapi.createGeneralization(DELETECOLLECTIONEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes DeleteCollectionEvent and Event.");
					}
				if (!raapi.isDirectSubClass(COPYCUTCOLLECTIONEVENT, EVENT))
					if (!raapi.createGeneralization(COPYCUTCOLLECTIONEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes CopyCutCollectionEvent and Event.");
					}
				if (!raapi.isDirectSubClass(COPYCOLLECTIONEVENT, EVENT))
					if (!raapi.createGeneralization(COPYCOLLECTIONEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes CopyCollectionEvent and Event.");
					}
				if (!raapi.isDirectSubClass(MOVELINESTARTPOINTEVENT, EVENT))
					if (!raapi.createGeneralization(MOVELINESTARTPOINTEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes MoveLineStartPointEvent and Event.");
					}
				if (!raapi.isDirectSubClass(MOVELINEENDPOINTEVENT, EVENT))
					if (!raapi.createGeneralization(MOVELINEENDPOINTEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes MoveLineEndPointEvent and Event.");
					}
				if (!raapi.isDirectSubClass(L2CLICKEVENT, EVENT))
					if (!raapi.createGeneralization(L2CLICKEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes L2ClickEvent and Event.");
					}
				if (!raapi.isDirectSubClass(LCLICKEVENT, EVENT))
					if (!raapi.createGeneralization(LCLICKEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes LClickEvent and Event.");
					}
				if (!raapi.isDirectSubClass(RCLICKEVENT, EVENT))
					if (!raapi.createGeneralization(RCLICKEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes RClickEvent and Event.");
					}
				if (!raapi.isDirectSubClass(NEWLINEEVENT, EVENT))
					if (!raapi.createGeneralization(NEWLINEEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes NewLineEvent and Event.");
					}
				if (!raapi.isDirectSubClass(NEWBOXEVENT, EVENT))
					if (!raapi.createGeneralization(NEWBOXEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes NewBoxEvent and Event.");
					}
				if (!raapi.isDirectSubClass(EXECTRANSFEVENT, EVENT))
					if (!raapi.createGeneralization(EXECTRANSFEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ExecTransfEvent and Event.");
					}
				if (!raapi.isDirectSubClass(NEWPINEVENT, EVENT))
					if (!raapi.createGeneralization(NEWPINEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes NewPinEvent and Event.");
					}
				if (!raapi.isDirectSubClass(CHANGEPARENTEVENT, EVENT))
					if (!raapi.createGeneralization(CHANGEPARENTEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ChangeParentEvent and Event.");
					}
				if (!raapi.isDirectSubClass(ACTIVATEDGREVENT, EVENT))
					if (!raapi.createGeneralization(ACTIVATEDGREVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ActivateDgrEvent and Event.");
					}
				if (!raapi.isDirectSubClass(CLOSEDGREVENT, EVENT))
					if (!raapi.createGeneralization(CLOSEDGREVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes CloseDgrEvent and Event.");
					}
				if (!raapi.isDirectSubClass(OKSTYLEDIALOGEVENT, EVENT))
					if (!raapi.createGeneralization(OKSTYLEDIALOGEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes OKStyleDialogEvent and Event.");
					}
				if (!raapi.isDirectSubClass(KEYDOWNEVENT, EVENT))
					if (!raapi.createGeneralization(KEYDOWNEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes KeyDownEvent and Event.");
					}
				if (!raapi.isDirectSubClass(NEWFREEBOXEVENT, EVENT))
					if (!raapi.createGeneralization(NEWFREEBOXEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes NewFreeBoxEvent and Event.");
					}
				if (!raapi.isDirectSubClass(NEWFREELINEEVENT, EVENT))
					if (!raapi.createGeneralization(NEWFREELINEEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes NewFreeLineEvent and Event.");
					}
				if (!raapi.isDirectSubClass(FREEBOXEDITEDEVENT, EVENT))
					if (!raapi.createGeneralization(FREEBOXEDITEDEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes FreeBoxEditedEvent and Event.");
					}
				if (!raapi.isDirectSubClass(FREELINEEDITEDEVENT, EVENT))
					if (!raapi.createGeneralization(FREELINEEDITEDEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes FreeLineEditedEvent and Event.");
					}
				if (!raapi.isDirectSubClass(TOOLBARELEMENTSELECTEVENT, EVENT))
					if (!raapi.createGeneralization(TOOLBARELEMENTSELECTEVENT, EVENT)) {
						setRAAPI(null, null, false); // freeing references initialized so far...
						throw new ElementReferenceException("Error creating a generalization between classes ToolbarElementSelectEvent and Event.");
					}
			...
			if (COMMAND_GRAPHDIAGRAM == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
				throw new ElementReferenceException("Error obtaining a reference for the association end graphDiagram of the class Command.");
			}
			...
			if ((POPUPDIAGRAM_POPUPELEMENT == 0) && insertMetamodel) {
				POPUPDIAGRAM_POPUPELEMENT = raapi.createAssociation(POPUPDIAGRAM, POPUPELEMENT, "popUpDiagram", "popUpElement", false);
			}
			if (POPUPDIAGRAM_POPUPELEMENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((PALETTE_PALETTEELEMENT == 0) && insertMetamodel) {
				PALETTE_PALETTEELEMENT = raapi.createAssociation(PALETTE, PALETTEELEMENT, "palette", "paletteElement", false);
			}
			if (PALETTE_PALETTEELEMENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((TOOLBAR_TOOLBARELEMENT == 0) && insertMetamodel) {
				TOOLBAR_TOOLBARELEMENT = raapi.createAssociation(TOOLBAR, TOOLBARELEMENT, "toolbar", "toolbarElement", false);
			}
			if (TOOLBAR_TOOLBARELEMENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((GRAPHDIAGRAM_COLLECTION == 0) && insertMetamodel) {
				GRAPHDIAGRAM_COLLECTION = raapi.createAssociation(GRAPHDIAGRAM, COLLECTION, "graphDiagram", "collection", false);
			}
			if (GRAPHDIAGRAM_COLLECTION == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((GRAPHDIAGRAM_ELEMENT == 0) && insertMetamodel) {
				GRAPHDIAGRAM_ELEMENT = raapi.createAssociation(GRAPHDIAGRAM, ELEMENT, "graphDiagram", "element", false);
			}
			if (GRAPHDIAGRAM_ELEMENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((COLLECTION_ELEMENT == 0) && insertMetamodel) {
				COLLECTION_ELEMENT = raapi.createAssociation(COLLECTION, ELEMENT, "collection", "element", false);
			}
			if (COLLECTION_ELEMENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((ELEMENT_COMPARTMENT == 0) && insertMetamodel) {
				ELEMENT_COMPARTMENT = raapi.createAssociation(ELEMENT, COMPARTMENT, "element", "compartment", false);
			}
			if (ELEMENT_COMPARTMENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((NODE_COMPONENT == 0) && insertMetamodel) {
				NODE_COMPONENT = raapi.createAssociation(NODE, NODE, "container", "component", false);
			}
			if (NODE_COMPONENT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
			...
			if ((NODE_PORT == 0) && insertMetamodel) {
				NODE_PORT = raapi.createAssociation(NODE, PORT, "node", "port", false);
			}
			if (NODE_PORT == 0) {
				setRAAPI(null, null, false); // freeing references initialized so far...
