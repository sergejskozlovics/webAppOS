  	...
  	implements RAAPIReferenceWrapper
{
	...
	// package-visibility:
	Frame(GraphDiagramEngineMetamodelFactory _factory)
	{
		factory = _factory;
	...
	public Frame(GraphDiagramEngineMetamodelFactory _factory, long _rObject, boolean _takeReference)
	{
		factory = _factory;
		rObject = _rObject;
	...
	public static Iterable<? extends Frame> allObjects(GraphDiagramEngineMetamodelFactory factory)
	{
	...
	public static Frame firstObject(GraphDiagramEngineMetamodelFactory factory)
	{
		long it = factory.raapi.getIteratorForAllClassObjects(factory.FRAME);
		if (it == 0)
			return null;
	...
	{
		return new GraphDiagramEngineMetamodel_RAAPILinkedObjectsList<GraphDiagram>(factory, rObject, factory.FRAME_GRAPHDIAGRAM); 
	}
	public boolean setGraphDiagram(GraphDiagram value)
	{
		boolean ok = true;
		long it = factory.raapi.getIteratorForLinkedObjects(rObject, factory.FRAME_GRAPHDIAGRAM);
		...
		if (it != 0) {
			...
			for (Long rLinked : list)
				if (!factory.raapi.deleteLink(rObject, rLinked, factory.FRAME_GRAPHDIAGRAM))
					ok = false;
		}
		if (value != null)
			if (!factory.raapi.createLink(rObject, value.rObject, factory.FRAME_GRAPHDIAGRAM))
				ok = false;
		return ok;
	}
}
