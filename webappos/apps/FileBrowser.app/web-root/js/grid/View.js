define([
	...
	return declare(null, {
		...
		services: {
			thumbnail: require.toUrl('rfe-php') + '/image.php'
		},
		...
		cellRenderers: {
			...
			// style="word-wrap:break-word;" by SK
			list: {
				name: function(obj, data, containerEl) {
					containerEl.innerHTML = '<span class="' + (obj.dir ? 'dijitFolderClosed' : 'dijitLeaf') + '"></span><span style="word-wrap:break-word;">' + obj.name + '</span>';
				}
			},
			icons: {
				name: function(obj, data, containerEl) {
					containerEl.innerHTML = '<span class="dgrid-column-name field-name" style="word-wrap:break-word;">' + obj.name + '</span>';
				}
			}
		},
...
/*			switch (mime) {
		...
		cell: function(target, columnId) {
			...
			if (this.view === 'icons' && this.row(target)) {
				if (this.row(target).element) // if added by SK
